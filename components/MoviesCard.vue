<template>
        ...
        <div class="movie-card" v-for="movie in movies" :key="movie.id">  
          <a :href="`/movie-details/${movie.id}`"> 
            ...
            <div class="movie-info">
              <h3 class="movie-title-font-style mt-2">{{ movie.title }}</h3>
              <p class="movie-subinfo-font-style mt-2">{{ movie.releaseDate }} | {{ movie.duration }} </p>
  ...
  export default {
    ...
    async mounted() {
      const response = await fetch('https://api.themoviedb.org/3/movie/popular?api_key=ddaac02629994be6d82db65948ef5a58')
